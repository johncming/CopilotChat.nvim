...
function Copilot:with_models(on_done, on_error)
...
function Copilot:with_claude(model, on_done, on_error)
  if self.claude_enabled or not vim.startswith(model, 'claude') then
    on_done()
    return
  end
